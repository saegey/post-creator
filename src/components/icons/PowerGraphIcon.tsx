const PowerGraphIcon = ({ color }: { color?: string }) => (
  <svg
    width="100%"
    height="100%"
    viewBox="0 0 236 246"
    className="childButton"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M30 16L30 1L6.55671e-07 0.999999L0 16L30 16ZM0 16L-9.17939e-06 226L30 226L30 16L0 16Z"
      fill={color ? color : `var(--theme-ui-colors-text)`}
    />
    <path
      d="M220 246H235V216H220V246ZM220 216L-2.38419e-06 216V246L220 246V216Z"
      fill={color ? color : `var(--theme-ui-colors-text)`}
    />
    <path
      d="M49 10C153.5 111 119.5 194.5 225.5 198"
      stroke={color ? color : `var(--theme-ui-colors-text)`}
      strokeWidth="20"
      strokeMiterlimit="11.4737"
      strokeLinecap="round"
    />
    <path
      d="M180 24L160 62.5H188.5L167 97"
      stroke={color ? color : `var(--theme-ui-colors-text)`}
      strokeWidth="20"
      strokeMiterlimit="11.4737"
      strokeLinecap="round"
    />
  </svg>
);

export default PowerGraphIcon;

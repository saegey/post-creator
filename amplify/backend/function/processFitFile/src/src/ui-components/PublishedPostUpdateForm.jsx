...
import { getPublishedPost } from "../graphql/queries";
import { updatePublishedPost } from "../graphql/mutations";
export default function PublishedPostUpdateForm(props) {
  const {
    id: idProp,
    publishedPost: publishedPostModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = publishedPostRecord
      ? { ...initialValues, ...publishedPostRecord }
      : initialValues;
    ...
    setAuthor(
      typeof cleanValues.author === "string" || cleanValues.author === null
        ? cleanValues.author
        : JSON.stringify(cleanValues.author)
    );
    ...
    setTimeSeriesFile(cleanValues.timeSeriesFile);
    setPowerZoneBuckets(
    ...
    setWebscorerResults(
      typeof cleanValues.webscorerResults === "string" ||
        cleanValues.webscorerResults === null
        ? cleanValues.webscorerResults
        : JSON.stringify(cleanValues.webscorerResults)
    );
    setCrossResults(
      typeof cleanValues.crossResults === "string" ||
        cleanValues.crossResults === null
        ? cleanValues.crossResults
        : JSON.stringify(cleanValues.crossResults)
    );
    setOmniResults(
      typeof cleanValues.omniResults === "string" ||
        cleanValues.omniResults === null
        ? cleanValues.omniResults
        : JSON.stringify(cleanValues.omniResults)
    );
    setRunSignupResults(
      typeof cleanValues.runSignupResults === "string" ||
        cleanValues.runSignupResults === null
        ? cleanValues.runSignupResults
        : JSON.stringify(cleanValues.runSignupResults)
    );
    setRaceResultsProvider(cleanValues.raceResultsProvider);
    setOriginalPostId(cleanValues.originalPostId);
    setErrors({});
  };
  const [publishedPostRecord, setPublishedPostRecord] = React.useState(
    publishedPostModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getPublishedPost.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPublishedPost
        : publishedPostModelProp;
      setPublishedPostRecord(record);
    };
    queryData();
  }, [idProp, publishedPostModelProp]);
  React.useEffect(resetStateValues, [publishedPostRecord]);
  const validations = {
    title: [{ type: "Required" }],
    gpxFile: [],
    images: [{ type: "JSON" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          timeSeriesFile: timeSeriesFile ?? null,
          ...
          webscorerResults: webscorerResults ?? null,
          crossResults: crossResults ?? null,
          omniResults: omniResults ?? null,
          runSignupResults: runSignupResults ?? null,
        ...
        try {
          ...
          await API.graphql({
            query: updatePublishedPost.replaceAll("__typename", ""),
            variables: {
              input: {
                id: publishedPostRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "PublishedPostUpdateForm")}
      {...rest}
    >
      <TextField
        label="Title"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || publishedPostModelProp)}

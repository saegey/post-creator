...
const { Parameters } = await (new aws.SSM())
  .getParameters({
    Names: ["SENTRY_AUTH_TOKEN"].map(secretName => process.env[secretName]),
    WithDecryption: true,
  })
  .promise();
...
exports.handler = async (event) => {
  ...
  try {
    ...
    var cognitoAuthenticationProvider =
      event.requestContext.identity.cognitoAuthenticationProvider;

    await attachIoTPolicyToFederatedIdentity(IOT_POLICY, identityId);

    var { userPoolId, username } = await getUserPoolInfoFromAuthProviderString(
...
async function attachIoTPolicyToFederatedIdentity(policyName, target) {
  console.log(
    `Attaching ${policyName} IoT policy to federated ID ${target}...`
  );
...
async function getUserPoolInfoFromAuthProviderString(authProviderString) {
  console.log(
    `Extracting Cognito user ID (aka 'sub') from auth string ${authProviderString}...`
  );
  ...
  console.log(
    `Calling Cognito.listUsers() to find username from user ID ${userPoolUserId}...`
  );
